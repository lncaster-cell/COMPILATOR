...
- `al_route_index` — опциональный индекс точки маршрута (`0..1023`) для явного порядка.
  - Если хотя бы у одной waypoint с тегом маршрута задан валидный `al_route_index`, тег работает в indexed-режиме.
  - В indexed-режиме waypoint без валидного `al_route_index` пропускаются.
  - Если индекс не задан ни у одной waypoint маршрута, порядок строится в dense/fallback-режиме (как раньше).
  - Legacy-флаг `al_route_index_set` больше не требуется как часть контент-контракта.
