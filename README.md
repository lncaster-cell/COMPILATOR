...
- `al_adjacent_areas` (string CSV) — список tag соседних area, например `CITY_A_02,CITY_A_03`;
- `al_area_heat` (int enum: `0=COLD`, `1=WARM`, `2=HOT`) — текущее «тепловое» состояние area.

Рекомендованный runtime-policy:
- если area-источник в состоянии `HOT`, прямые соседи из `al_adjacent_areas` могут быть повышены до `WARM`;
- interior-area (не пограничная для активных игроков) по умолчанию считается кандидатом в `COLD`, если нет явного wake-триггера;
- если adjacency-конфиг неполный/битый, система должна логировать fallback в debug и продолжать работу в локальном режиме (без silent ошибок и без каскадного прогрева соседей).
